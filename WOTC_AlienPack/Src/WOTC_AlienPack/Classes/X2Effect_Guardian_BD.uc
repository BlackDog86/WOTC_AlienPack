class X2Effect_Guardian_BD extends X2Effect_Persistent;
...
function bool PostAbilityCostPaid(XComGameState_Effect EffectState, XComGameStateContext_Ability AbilityContext, XComGameState_Ability kAbility, XComGameState_Unit SourceUnit, XComGameState_Item AffectWeapon, XComGameState NewGameState, const array<name> PreCostActionPoints, const array<name> PreCostReservePoints)
{
	...
	local int RandRoll;
	local UnitValue GuardianActivatedValue;

	//Check to see whether the guardian unitvalue is already set (i.e. the ability has already been activated once this turn - if so exit) 
	SourceUnit.GetUnitValue('GuardianActivations', GuardianActivatedValue);
	if (GuardianActivatedValue.fValue > 0)
	{
	return false;
	}
	...
	if (SourceUnit.ReserveActionPoints.Length != PreCostReservePoints.Length && AbilityContext.IsResultContextHit() && AllowedAbilities.Find(kAbility.GetMyTemplate().DataName) != INDEX_NONE)
	{
		...
		if (AbilityState != none)
		{
			...
			if (RandRoll < ProcChance)
			{
				...
				// If we're activating guardian, set a unit value on the unit for this turn
				SourceUnit.SetUnitFloatValue('GuardianActivations', 1, eCleanup_BeginTurn);
				EventMgr = `XEVENTMGR;
