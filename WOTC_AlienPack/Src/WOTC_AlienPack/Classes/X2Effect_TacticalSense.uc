...
class X2Effect_TacticalSense extends X2Effect_Persistent config (WOTC_AlienPack);

var config int BD_TACTICAL_SENSE_DEF_BONUS_PER_ENEMY;
var config int BD_TACTICAL_SENSE_MAX_DEF_BONUS;
...
function GetToHitAsTargetModifiers(XComGameState_Effect EffectState, XComGameState_Unit Attacker, XComGameState_Unit Target, XComGameState_Ability AbilityState, class<X2AbilityToHitCalc> ToHitType, bool bMelee, bool bFlanking, bool bIndirectFire, out array<ShotModifierInfo> ShotModifiers)
{
  ...
  if (BadGuys > 0)
  {
    ...
    ShotInfo.Value = -1 * (Clamp (BadGuys * default.BD_TACTICAL_SENSE_DEF_BONUS_PER_ENEMY, 0, default.BD_TACTICAL_SENSE_MAX_DEF_BONUS));
